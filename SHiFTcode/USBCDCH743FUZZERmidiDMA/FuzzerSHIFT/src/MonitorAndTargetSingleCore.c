...
char redzone1[MIDI_BUF_SIZE] __attribute__( ( aligned( next_power_of_2(MIDI_BUF_SIZE)  ) ) );

char midiBuffer[MIDI_BUF_SIZE] __attribute__( ( aligned( next_power_of_2(MIDI_BUF_SIZE)  ) ) );    /* for debugging */ //buffer

char redzone2[MIDI_BUF_SIZE] __attribute__( ( aligned( next_power_of_2(MIDI_BUF_SIZE)  ) ) );
...
//MIDIMsgBuilder_State_t lastState; //global variable
...
extern void MIDI_note_on_do(void *data, MIDIMsg *msg);
extern void MIDI_note_off_do(void *data, MIDIMsg *msg);
...
static void spawnNewTarget()
{
   ...
   TaskParameters_t targetTaskParameters =
   {
        	...
        	.xRegions		=	{
									...
									{  midiBuffer, next_power_of_2(MIDI_BUF_SIZE),  portMPU_REGION_READ_WRITE},
									{  &midiMsgBuilder, next_power_of_2(sizeof(MIDIMsgBuilder)) ,  portMPU_REGION_READ_WRITE},
									{  &midiRouter,next_power_of_2(sizeof(MIDI_Router_Standard)) ,  portMPU_REGION_READ_WRITE},
...
static void fuzzerTask( void * pvParameters )
{
	...
	memset(redzone4,0xbb,sizeof(redzone4));
	...
	for( ; ; )
	{
		...
		ulTaskNotifyTakeIndexed(1,pdTRUE, portMAX_DELAY);
		{
			...
			// and channels that can trigger hardware errors and hang during fuzzing
			 xTaskNotify(AFLfuzzer.xTaskTarget,sizeRX,eSetValueWithOverwrite);
