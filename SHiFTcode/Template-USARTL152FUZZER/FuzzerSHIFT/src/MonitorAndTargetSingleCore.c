...
static void spawnNewTarget( )
{
   ...
   TaskParameters_t targetTaskParameters =
   {
        	...
        	.xRegions		=	{
        			                //{ 0, 0, 0 }, // AFL bitmap, diff buffer, TX diff buffer, this region is shareable
									{ (void *)(0x20010000), 16*1024, portMPU_REGION_READ_WRITE  },
									{  AFLfuzzerRegion, AFLINPUTREGION_SIZE, portMPU_REGION_READ_WRITE },
									{ __user_heap_start__, 1024*2, portMPU_REGION_READ_WRITE  },

   							 	    //{ paflbitmap, AFL_BITMAP_SIZE_BYTES, portMPU_REGION_READ_WRITE},    // This is necessary because AFL_BITMAP is in the TCM region
   								    //{ ( void * )0x20010000, 16*1024, portMPU_REGION_READ_WRITE },     // shadow memory
