...
//global var used by other files
...
void send_forkserver_error(int error) {
  ...
  u32 status;
#ifdef DBG
  fprintf(dbg_file,"error happens:%d\n",error);
  fflush(dbg_file);
#endif
...
static void __afl_end_testcase(int ret_status) {
...
int main(int argc, char *argv[]) {
  ...
  fprintf(dbg_file,"start dbg\n");
  fflush(dbg_file);
#endif
  while ((opt = getopt(argc, argv, "+c:w:t:")) > 0){
#ifdef DBG
  fprintf(dbg_file,"opt: %d\n",opt);
  fflush(dbg_file);
#endif
    switch (opt)
    {
    ...
    case 't': { /* timeout */
          u8 suffix = 0;
#ifdef DBG
              fprintf(dbg_file,"optarg:%s\n",optarg);
              fflush(dbg_file);
#endif
          if (sscanf(optarg, "%u%c", &exec_tmout, &suffix) < 1 ||
              optarg[0] == '-') FATAL("Bad syntax used for -t");
#ifdef DBG
              fprintf(dbg_file,"exec_tmout:%d,suffix %c\n",exec_tmout, suffix);
              fflush(dbg_file);
#endif
...
#ifdef DBG
  fprintf(dbg_file,"serial port open ok\n");
  fflush(dbg_file);
#endif
...
#ifdef DBG
  fprintf(dbg_file,"shm and forkserver ok\n");
  fflush(dbg_file);
#endif
  while ((len = __afl_next_testcase(buf, sizeof(buf))) > 0) {
    sendInputs((uint8_t *) buf, len);
    uint32_t exit_status;
    getFeedback(&exit_status, (u8*)__afl_area_ptr);
