...
/*
	  ...
	  if( *Len>0 &&  hUsbDeviceFS.dev_state == USBD_STATE_CONFIGURED )
	  {
	      ...
	      if(inputLength)
	      {
	  	     while(inputAFL.u32available < inputLength && u32copied < (*Len - offset))
	  	     {
	  	  	    RingAdd(&inputAFL, Buf[u32copied + offset]);
	  	  	    u32copied++;
	  	     }
	  	     if(inputLength == inputAFL.u32available)
	  	     {
	  	    	bRXcomplete = 1;
	  	    	timespan = HAL_GetTick();
	  	     }
